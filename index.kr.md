...
컴퓨터로 제작된 지도의 근간은 데이터이다. 데이터는 특정한 용도에 국한되지 않으며, 어떠한 형태든 될 수 있다. 데이터는 점이나 다각형의 형태를 가지고 있을 수도 있고, 혹은 사진과 같은 이미지의 형태일 수도 있다.
...
상징화는 데이터들이 지도위의 시각적인 요소가 되어가는 과정을 멋드러지게 일컫는 말이다.
...
상징화 테크닉은 3디를 포함한 그래픽으로 나타낼 수 있는 모든 것을 포함한다. 몇 가지 예를 살펴보자.
