...
地理データは **ラスタ** と **ベクタ** のどちらかの方法で表現されます。ラスタはデータがピクセルとして格納され、ベクタではジオメトリとして格納されます。実際にデータを利用する際、この2つの形式はしばしば同時に利用されます。例えばラスタ形式で提供される衛星画像の上に、ベクタデータとして道路データを重ねて表示する方法はたいへん一般的です。
